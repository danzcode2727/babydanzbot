...
prefix = ','
blocked = []   
userpremium = ["6289686610207@s.whatsapp.net","6281253491026@s.whatsapp.net","6285338319214@s.whatsapp.net","6282111829914@s.whatsapp.net"]
limitawal = 20
memberlimit = 2
cr = '* D A N Z B O T  V E R I F I E D'

/*
]=====> SETTINGS <=====[
*/
const ownerNumber = ["6289686610207@s.whatsapp.net","6281236547839@s.whatsapp.net"]
const botName = '*"D A N Z  B O T"*'
const ownerName = '*D A N Z !*'
...
*/
				...
				ramlan = body.slice(8)
				reply('「❗」WAIT, MASIH NULIS DULU YAKAN')
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis2 Danz baik hati`)
				laysha = body.slice(8)
				reply('「❗」WAIT, MASIH NULIS DUMLU YAKAN')
				...
				client.sendMessage(from, ''+tes+'\n\n_By : ⸸Danz⸸Panutanque._', text, { quoted: mek })
				await limitAdd(sender)
				break
// NINJALOGO				
				case 'ninjalogo':
...
// PORNHUB
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}pornhub Danz & Hub`)
				reply(ind.wait())
...
//GEMBOK TEXT
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply(`[❗] Contoh : ${prefix}gemboktext 11 01 2021 & Danz dan Ella`)
					reply(ind.wait())
...
*/
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					const bisa =['Tentu Saja Bisa! Kamu Adalah Orang Paling Homky','Gak Bisa aowkwkwk', 'Bisa','Tidak','Hmmmm', 'Coba Lagi', 'tanya ama bapakau','Ulangi Lagi']
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
					apakah = body.slice(1)
					const apa =['Iya','Tidak','Bisa Jadi','Coba Ulangi','Gak Bisa Aowkaowk']
...
*/		 
				...
				if (!isRegistered) return reply(ind.noregis())
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Atas izin admin grup kamu akan saya tendang 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Atas izin admin grup kamu akan saya tendang @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
			    case 'kudeta':
        ...
        if (isBanned) return reply(ind.baned())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))		
					...
					mentions(`╔═══✪ Tag By *${pushname}* ✪══`+ teks +'╚═══〘 D A N Z  B O T〙═══', members_id, true)
					break
                case 'indohot':
              if (isBanned) return reply(ind.baned())    
              if (!isRegistered) return reply(ind.noregis())
           ...
           case 'demote':
                if (!isRegistered) return reply(ind.noregis())           
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('*TAG TARGET YANG INGIN DITURUNKAN MENJADI ADMIN !*')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
                ...
                if (!isRegistered) return reply(ind.noregis())				
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('*TAG TARGET YANG INGIN DINAIKKAN MENJADI ADMIN!*')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Yeee🥳 Kamu naik jabatan menjadi admin>_< :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner Danz`)
                      if (isOwner) {
                      const one = 999999999
